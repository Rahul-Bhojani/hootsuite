...
router.post('/register', async (req, res) => {
    ...
    if (req.body.email == undefined || req.body.username == undefined || req.body.role == undefined || req.body.firstname == undefined || req.body.lastname == undefined) { return res.status(400).send({ msg: "All Filed is Required" }) }
    //Email Format Check
    ...
    if (validate.lengthCheck(req.body.firstname, 2)) return res.status(400).send({ msg: 'FirstName length min 2 required!!!' })
    if (validate.lengthCheck(req.body.lastname, 2)) return res.status(400).send({ msg: 'LastName length min 2 required!!!' })
    if (req.body.mobileno != undefined) {
        if (validate.NumberlengthCheck(req.body.mobileno, 10) || isNaN(req.body.mobileno)) return res.status(400).send({ msg: 'Enter valid Mobile Number' })
    }
    if (req.body.designation != undefined) if (validate.lengthCheck(req.body.designation, 2)) return res.status(400).send({ msg: 'designation length min 2 required!!!' })
...
router.patch('/profile', auth, async (req, res) => {
    if (req.body.firstname != undefined) if (validate.lengthCheck(req.body.firstname, 3)) return res.status(400).send({ msg: 'FirstName length min 3 required!!!' })
    if (req.body.lastname != undefined) if (validate.lengthCheck(req.body.lastname, 3)) return res.status(400).send({ msg: 'lastname length min 3 required!!!' })
    if (req.body.officeno != undefined) {
        if (validate.NumberlengthCheck(req.body.officeno, 10) || isNaN(req.body.officeno)) return res.status(400).send({ msg: 'Enter valid Office Number' })
    }
    if (req.body.mobileno != undefined) {
        if (validate.NumberlengthCheck(req.body.mobileno, 10) || isNaN(req.body.mobileno)) return res.status(400).send({ msg: 'Enter valid Mobile Number' })
    }
    if (req.body.designation != undefined) if (validate.lengthCheck(req.body.designation, 2)) return res.status(400).send({ msg: 'designation length min 2 required!!!' })
    if (req.body.address != undefined) if (validate.lengthCheck(req.body.address, 3)) return res.status(400).send({ msg: 'address length min 3 required!!!' })
...
router.patch('/password', auth, async (req, res) => {
    ...
    if (req.body.password != undefined) {
        ...
        if (!validPass) { return res.status(400).send({ msg: " Old password not valid!!!" }) }
        else {
            const salt = await bcrypt.genSalt(10);
...
router.get('/count', async (req, res) => {
    ...
    const staffWithoutTask = await User.countDocuments({ task: { $eq: null }, role: { $ne: 1 } })
    res.status(200).send({ staffMember, tasks, staffWithoutTask, staffAdmin, staffHr })
})
